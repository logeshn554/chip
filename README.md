...
A research system for autonomous hardware architecture search, synthesizable SystemVerilog design, and physical constraint satisfaction driven by local open-weight LLMs (**Qwen-14B** / `qwen2.5:14b` via **Ollama**, **Transformers**, or **vLLM**).
...
├── Target AI Workload (e.g. Qwen-14B-INT4 quantized parameters)
...
┌────────────────────────────────────────────────────────┐
│                   External Policy                      │
│     (Qwen-14B / PPO / GRPO Policy Generator)           │
└──────────────────────────┬─────────────────────────────┘
...
> `HardwareDesignEnv` is deliberately designed as the underlying Gymnasium execution environment. It does not replace or embed the agent policy. Qwen-14B (or any external RL policy, PPO, or GRPO actor) acts as the decision-making policy driving the environment.

---

## 3. Supported Action System (17 Predefined Typed Actions)
...
| `GENERATE_TESTBENCH`| Create non-heldout verification harness | Allowed only in `./rtl/generated/testbenches` |
...
## 4. Two-Level Reasoning & Hardware Architecture Search
...
## 5. Targeted Web Research via ScrapeGraphAI

The web research pipeline replaces naive search with a domain-bounded, injection-defended extraction pipeline:
- **Primary ScrapeGraphAI Pipeline** (`scraping/scrapegraph_adapter.py`): Compact technical extraction using `SmartScraperGraph` directly over target URLs without redundant preliminary fetches. Model is aligned with **Qwen-14B** (`qwen2.5:14b`).
